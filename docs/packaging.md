...
- **Camera recording**: Requires `rpicam-vid` (modern Raspberry Pi OS) or `libcamera-vid` (legacy rpicam-apps), and ffmpeg for reliable MP4 finalization when stopping recordings. The .deb package lists ffmpeg as a dependency. The service user is added to `video` and `render` (if present) for `/dev/media*` and DMA buffer access. **Restart the service or reboot** after install/upgrade for group membership to take effect.
