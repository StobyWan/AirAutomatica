...
- **Camera recording**: Requires `rpicam-vid` (modern Raspberry Pi OS) or `libcamera-vid` (legacy rpicam-apps). Not a package dependency. The service user is added to `video` and `render` (if present) for `/dev/media*` and DMA buffer access. **Restart the service or reboot** after install/upgrade for group membership to take effect.
